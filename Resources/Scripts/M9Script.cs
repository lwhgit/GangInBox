...
public class M9Script : Weapon {
    ...
    public override bool Shoot(Vector3 point) {
        ...
        if(shootable) {
            ...
            obj.transform.position = gameObject.transform.position + gameObject.transform.right * 0.04f + gameObject.transform.up * 0.08f - gameObject.transform.forward * 0.02f;
            obj.transform.rotation = gameObject.transform.rotation;
            obj.AddComponent<BoxCollider>();
    ...
    protected override void Rebound() {
        ...
        if(reboundRotX > 0) {
            reboundRotX -= 20f * Time.deltaTime;
        }
